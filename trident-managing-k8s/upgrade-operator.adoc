...
* 您應該使用執行的Kubernetes叢集 link:requirements.html["版本1.19 - 1.24"^]。
...
----
. （選用）如果您要修改安裝參數、請編輯 `TridentOrchestrator` 安裝Trident時建立的物件。這可能包括修改自訂Trident映像、從私有映像登錄擷取容器映像、啟用偵錯記錄或指定映像提取機密等變更。
